...
import { generateConversationReport } from '@/services/deepseek';
...
export const useLeave = () => {
  ...
  return async function () {
    // Immediately perform actions to leave the room and update UI
    await Promise.all([
    ...
    // Create a copy of msgHistory before clearing it
    const currentMsgHistory = [...msgHistory];
    dispatch(clearHistoryMsg());
    dispatch(clearCurrentMsg());
    dispatch(updateAIGCState({ isAIGCEnable: false }));

    // Generate and save the conversation report asynchronously
    if (currentMsgHistory.length > 0) {
      dispatch(setGeneratingReport(true));
      generateConversationReport(currentMsgHistory)
        .then(report => {
          dispatch(updateConversationReport(report));
          console.log('对话分析报告已异步生成', report);
        })
        .catch(error => {
          console.error('异步生成对话报告失败:', error);
          dispatch(updateConversationReport('生成报告时出现错误，请稍后再试。'));
        });
    }
  };
};
